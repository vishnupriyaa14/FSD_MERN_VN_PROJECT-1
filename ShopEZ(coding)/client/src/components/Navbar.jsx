...
  
  // Stores categories in their original casing (e.g., "Mobiles", "Sports-Equipment")
...
  try {
    const res = await axios.get('http://localhost:6001/api/products/fetch-categories');
    
    // Store the categories exactly as they come from the backend
...
// FIX: Revised handleSearch for robust, case-insensitive searching
const handleSearch = () => {
  const searchValue = productSearch.trim().toLowerCase();

  if (!searchValue) return;

  // Find a category where the lowercase original category includes the lowercase search term
  const matchedCategoryOriginalCasing = categories.find(cat =>
    // We compare the lowercase search value to the lowercase category string
    cat.toLowerCase().includes(searchValue)
  );

  if (matchedCategoryOriginalCasing) {
    // If a match is found, navigate using the EXACT original casing (e.g., "Mobiles")
    navigate(`/category/${matchedCategoryOriginalCasing}`);
    setNoResult(false);
  } else {
    setNoResult(true);
  }
